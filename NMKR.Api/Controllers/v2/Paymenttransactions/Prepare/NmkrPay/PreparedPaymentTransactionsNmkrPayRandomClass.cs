...
{
    public class PreparedPaymentTransactionsNmkrPayRandomClass : PreparedPaymentTransactionsNmkrPayClass
    {
        public PreparedPaymentTransactionsNmkrPayRandomClass(EasynftprojectsContext db, IConnectionMultiplexer redis, PaymentTransactionTypes PaymentTransactionType) : base(db,redis, PaymentTransactionType)
        {
        }
        ...
        public override ApiErrorResultClass CheckParameter(CreatePaymentTransactionClass paymenttransaction, Nftproject project, ApiErrorResultClass result, out int statuscode)
        {
            ...
            statuscode = 0;
            if (paymenttransaction.PaymentTransactionType != PaymentTransactionTypes.paymentgateway_nft_random && paymenttransaction.PaymentTransactionType!=PaymentTransactionTypes.nmkr_pay_random)
                return result;
                ...
                paymenttransaction.PaymentgatewayParameters.MintNfts.ReserveNfts.Any())
            {
                ...
                result.ErrorMessage = "You can not specify NFTs when you will sell them randomly";
                result.ResultState = ResultStates.Error;
                statuscode = 406;
                return result;
            }
