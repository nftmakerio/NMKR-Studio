...
{
    public class PreparedPaymentTransactionsNmkrPaySpecificClass : PreparedPaymentTransactionsNmkrPayClass
    {
        public PreparedPaymentTransactionsNmkrPaySpecificClass(EasynftprojectsContext db, IConnectionMultiplexer redis, PaymentTransactionTypes PaymentTransactionType) : base(db, redis, PaymentTransactionType)
        {
        }
        ...
        public override ApiErrorResultClass CheckParameter(CreatePaymentTransactionClass paymenttransaction, Nftproject project, ApiErrorResultClass result, out int statuscode)
        {
            ...
            statuscode = 0;
            if (paymenttransaction.PaymentTransactionType != PaymentTransactionTypes.paymentgateway_nft_specific && paymenttransaction.PaymentTransactionType!=PaymentTransactionTypes.nmkr_pay_specific)
                return result;
            ...
            out int statuscode)
        {
            ...
            if (paymenttransaction.PaymentgatewayParameters.MintNfts.ReserveNfts == null)
            {
                result.ErrorCode = 1044;
                result.ErrorMessage = "You have to specifiy the nfts in the PaymentgatewayParameters MintNfts structure";
                result.ResultState = ResultStates.Error;
                statuscode = 404;
                return result;
            }
            ...
            foreach (var ptcReserveSpecificNft in paymenttransaction.PaymentgatewayParameters.MintNfts.ReserveNfts)
            {
                ...
                if (ptcReserveSpecificNft.Tokencount > project.Maxsupply)
                {
                    result.ErrorCode = 1006;
                    result.ErrorMessage = $"Tokencount must below the max. supply of the tokens/nfts ({project.Maxsupply})";
                    result.ResultState = ResultStates.Error;
                    statuscode = 406;
                    return result;
                }
                ...
                {
                    var l = GlobalFunctions.GetPrice(db,redis, nftid, ptcReserveSpecificNft.Tokencount??1);
                    if (l != null)
                        lovelace += ((long) l);
                }
                ...
                var reserve = new PreparedpaymenttransactionsNft()
                {
                    ...
                    Lovelace = ptcReserveSpecificNft.Lovelace,
