...
namespace NMKR.Api.Controllers.v2.Paymenttransactions.Processing.SmartContract
{
    ...
    /// </summary>
    public class CancelSmartContractDirectSaleTransactionClass : ControllerBase, IProcessPaymentTransactionInterface
    {
        private readonly IConnectionMultiplexer _redis;

        public CancelSmartContractDirectSaleTransactionClass(IConnectionMultiplexer redis)
        {
            _redis = redis;
        }
        /// <summary>
        ...
        public async Task<IActionResult> ProcessTransaction(EasynftprojectsContext db, string apikey,string remoteipaddress, ApiErrorResultClass result, Preparedpaymenttransaction preparedtransaction, object postparameter1)
        {
          ...
          1500000, 0, null, null /*postparameter.Buyer.CollateralTxIn*/, out string errormessage, out AllTxInAddressesClass alltxin);

            PreparedpaymenttransactionsSmartcontractsjson ptsj = new()
            {
                PreparedpaymenttransactionsId = preparedtransaction.Id,
                Json = await StaticTransactionFunctions.FillJsonTemplateSellerDirectsaleAsync(db, preparedtransaction),
            ...
            SmartContractAuctionsParameterClass scapc = new()
            {
                ...
                collateraltxin = null, // postparameter.Buyer.CollateralTxIn, // The Collateral TX-In
           ...
           /*     smartcontractmemvalue = preparedtransaction.Smartcontracts.Memvalue,    // DO NOT INCLUDE - WE ARE USING THE BUILD COMMAND
            ...
            else
            {
                ...
                await GlobalFunctions.LogMessageAsync(db, $"Error while cancel smartcontract {preparedtransaction.Transactionuid}",
                    ...
                    (olddatum==null?"NO DATUM" : olddatum.Json) + Environment.NewLine + Environment.NewLine + ptsj.Redeemer);
        ...
        private string GetTokens(Preparedpaymenttransaction preparedtransaction)
        {
            string tokens = "";
            if (preparedtransaction.PreparedpaymenttransactionsNfts == null ||
                !preparedtransaction.PreparedpaymenttransactionsNfts.Any())
            {
            ...
            foreach (var nft in preparedtransaction.PreparedpaymenttransactionsNfts)
            {
                ...
                tokens += nft.Count + " " + nft.Policyid + "." + nft.Tokennamehex;
            }
