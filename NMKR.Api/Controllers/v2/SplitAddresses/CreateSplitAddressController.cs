...
namespace NMKR.Api.Controllers.v2.RoyaltySplitAddresses
{
    ...
    [ApiController]
    [ApiVersion("2")]
    public class CreateSplitAddressController : ControllerBase
    {
        private readonly IConnectionMultiplexer _redis;

        public CreateSplitAddressController(IConnectionMultiplexer redis)
        {
            _redis = redis;
        }
        /// <summary>
        ...
        public async Task<IActionResult> Post([OpenApiHeaderIgnore][FromHeader(Name = "authorization")] string apikey, int customerid, [FromBody] CreateSplitAddressClass createSplitAddress)
        {
           // string apikey = Request.Headers["authorization"];
            ...
            if (!ConsoleCommand.CheckIfAddressIsValid(db, createSplitAddress.MainAddress, GlobalFunctions.IsMainnet(),out string address,
            ...
            {
                if (!ConsoleCommand.CheckIfAddressIsValid(db, split.Address, GlobalFunctions.IsMainnet(), out string xaddress,
            ...
            var spa = new Splitroyaltyaddress()
            {
                Comment = createSplitAddress.Comment, 
                Created = DateTime.Now, 
                CustomerId = customerid, 
                Lovelace = 0,
                State = createSplitAddress.IsActive ? "active" : "notactive",
                Address = cn.Address,
                Skey = Encryption.EncryptString(cn.privateskey, salt),
                Vkey = Encryption.EncryptString(cn.privatevkey, salt),
                Salt = salt,
                Minthreshold = createSplitAddress.ThresholdInAda, 
            };
            ...
            Splitroyaltyaddressessplit spas = new()
            {
                ...
                State = "active",
                SplitroyaltyaddressesId = spa.Id,
                Created = DateTime.Now,
                IsMainReceiver = true,
                Percentage = 100,
            };
