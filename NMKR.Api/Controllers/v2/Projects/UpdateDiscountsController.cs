...
namespace NMKR.Api.Controllers.v2.Projects
{
    ...
    [ApiController]
    [ApiVersion("2")]
    public class UpdateDiscountsController : ControllerBase
    {
        private readonly IConnectionMultiplexer _redis;

        public UpdateDiscountsController(IConnectionMultiplexer redis)
        {
            _redis = redis;
        }
        ...
        /// <response code="200">The discounts was successfully updated</response>
        ...
        public async Task<IActionResult> Put([OpenApiHeaderIgnore][FromHeader(Name = "authorization")] string apikey, string projectuid,
            [FromBody] PriceDiscountClassV2[] discounts)
        {
           // string apikey = Request.Headers["authorization"];
            ...
            result = spf.CheckDiscounts(db, discounts, result);
            if (result.ResultState == ResultStates.Error)
                return StatusCode(406, result);

            // Delete the old pricelist
            string sql = $"delete from pricelistdiscounts where nftproject_id={project.Id}";
            await GlobalFunctions.ExecuteSqlWithFallbackAsync(db, sql);

            // Save the new pricelist
            await spf.SaveDiscounts(db, discounts, project.Id);
