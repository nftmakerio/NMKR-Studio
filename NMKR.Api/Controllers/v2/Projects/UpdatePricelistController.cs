...
namespace NMKR.Api.Controllers.v2.Projects
{
    ...
    [ApiController]
    [ApiVersion("2")]
    public class UpdatePricelistController : ControllerBase
    {
        private readonly IConnectionMultiplexer _redis;

        public UpdatePricelistController(IConnectionMultiplexer redis)
        {
            _redis = redis;
        }
        ...
        /// <response code="200">The pricelist was successfully updated</response>
        ...
        public async Task<IActionResult> Put([OpenApiHeaderIgnore][FromHeader(Name = "authorization")] string apikey, string projectuid,
            [FromBody] PricelistClassV2[] pricelist)
        {
           // string apikey = Request.Headers["authorization"];
            ...
            result = spf.CheckPricelists(db, pricelist, customer, result);
            if (result.ResultState == ResultStates.Error)
                return StatusCode(406, result);

            // Delete the old pricelist
            string sql = $"delete from pricelist where nftproject_id={project.Id}";
            await GlobalFunctions.ExecuteSqlWithFallbackAsync(db, sql);

            // Save the new pricelist
            await spf.SavePricelists(db, pricelist, project.Maxsupply,project.Id, customer.DefaultpromotionId);
