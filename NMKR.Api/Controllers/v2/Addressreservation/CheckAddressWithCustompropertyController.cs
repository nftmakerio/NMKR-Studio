...
namespace NMKR.Api.Controllers.v2.Addressreservation
{
    ...
    [ApiController]
    [ApiVersion("2")]
    public class CheckAddressWithCustompropertyController : ControllerBase
    {

        private readonly IConnectionMultiplexer _redis;

        public CheckAddressWithCustompropertyController(IConnectionMultiplexer redis)
        {
            _redis = redis;
        }
        ...
        /// <param Name="address">The custom property you want to check</param>
        ...
        /// <response code="404">The custom property was not found in our database or not assiged to this project</response>            
        ...
        [HttpGet("{projectuid}/{customproperty}")]
        [MapToApiVersion("2")]
        [SwaggerOperation(
            Tags = new[] { "Address reservation (sale)" }
        )]
        public async Task<IActionResult> Get([OpenApiHeaderIgnore][FromHeader(Name = "authorization")] string apikey, string projectuid, string customproperty)
        {
          //  string apikey = Request.Headers["authorization"];
            ...
            string apiparameter = projectuid + customproperty;
            ...
            return await Check(result, new(apikey, apifunction, apiparameter), projectuid, customproperty);
        }

        internal async Task<IActionResult> Check(ApiErrorResultClass result, CachedApiCallValues apivalues, string projectuid, string customproperty)
        {
            await using var db = new EasynftprojectsContext(GlobalFunctions.optionsBuilder.Options);
            {
                ...
                var adr = await (from a in db.Nftaddresses
                               ...
                               .ThenInclude(a => a.Nft)
                                                          where a.Customproperty == customproperty && a.NftprojectId == nftprojectid
                ...
                {
                    result.ErrorCode = 50;
                    result.ErrorMessage = "Custom property not found";
                    result.ResultState = ResultStates.Error;
                    CheckCachedAccess.SetCachedResult(_redis, apivalues, 404, result);
                    await db.Database.CloseConnectionAsync();
                    return NotFound(result);
