...
{
    public class CheckFiatMintAndSendOrders : IBackgroundServices
    {
            ...
            do
            {
                ...
                try
                {
                    mas = await (from a in db.Mintandsends
                            ...
                            .AsSplitQuery()
                        where a.State == "execute" && a.Usecustomerwallet == false && a.Remintandburn == false && a.Coin== Coin.ADA.ToString()
                ...
                foreach (var mas2 in mas1)
                {
                    ...
                    foreach (var mintandsend in mas2)
                    {
                        ...
                        if (!string.IsNullOrEmpty(md.Metadata))
                        {
                            ...
                            {
                                await StaticBackgroundServerClass.LogAsync(db,
                                    $"Adding FIAT Mint and Send - Metadata too large : {mas2.Key.Projectname} - {mas2.Key.Id} - {mintandsend.Receiveraddress} - {mintandsend.Reservationtoken} - {metadatasize} - Count: {nmasc.CountNfts}",
                                    mdjson, serverid);
                                break;
                            }
                ...
                foreach (var newMintAndSendClass in nmascl)
                {
                    ...
                    var paywallet = await GlobalFunctions.GetNmkrPaywalletAndBlockAsync(db, 0,"CheckFiatMintAndSendOrders", newMintAndSendClass.NftReservations.FirstOrDefault()?.Reservationtoken); 

                    if (paywallet == null)
                    {
                        await StaticBackgroundServerClass.LogAsync(db,
                            $"Fiat Mint and Send - All Paywallets are blocked - waiting: {newMintAndSendClass.NftProject.Projectname} - {newMintAndSendClass.NftProject.Id}",
                            "", serverid);
                        continue;
                    }

                    await StaticBackgroundServerClass.LogAsync(db,
                        $"Fiat Mint and Send - Account is ok - start minting: {newMintAndSendClass.NftProject.Projectname} - {newMintAndSendClass.NftProject.Id} {newMintAndSendClass.NftReservations.Count()}",
                        "", serverid);
                    ...
                    foreach (var res in newMintAndSendClass.NftReservations.OrEmptyIfNull())
                    {
                        if (res.Nft == null)
                        {
                            failed = true;
                        ...
                        if (!(bfq + res.Tc > newMintAndSendClass.NftProject.Maxsupply)) continue;

                        await StaticBackgroundServerClass.LogAsync(db,
                            $"FAILED - MintNfts - NFT was already sold - found on Blockfrost for - NftId: {res.NftId} - {res.Tc}",
                            "", serverid);
                    ...
                    if (failed)
                    {
                        ...
                        await SetNewMintandSendState(db, redis, newMintAndSendClass, "error",
                            new() {LogFile = "Mint and send failed"}, serverid, cancellationToken);
                    ...
                    /*   string s = ConsoleCommand.MintAndSend(db, redis, newMintAndSendClass, paywallet.Address, payskey,
                           ConsoleCommand.GetNodeVersion(), mainnet, true, false, newMintAndSendClass.NftProject.Mintandsendminutxo, false, out var buildtransaction);*/
                    ...
                    {
                        await StaticBackgroundServerClass.LogAsync(db,
                            $"FIAT Mint and send was error - releasing the Tokens: {newMintAndSendClass.NftProject.Projectname} - {newMintAndSendClass.NftProject.Id} {newMintAndSendClass.NftReservations.Count()} {paywallet.Address}",
                            "", serverid);
            ...
            string state, BuildTransactionClass buildtransaction, int serverid, CancellationToken cancellationToken)
        {
            foreach (var mas in newMintAndSendClass.MintAndSends)
            {
                ...
                if (mas.Onlinenotification)
                {
                    if (state == "canceled")
                    {
                        Onlinenotification on = new()
                        {
                            ...
                            Notificationmessage =
                                $"There was an error while minting the nft(s) to the address {mas.Receiveraddress}",
                            State = "new",
                            Color = "error"
                        };
                    }
                    ...
                    if (state == "success")
                    {
                        Onlinenotification on = new()
                        {
                            ...
                            CustomerId = mas.CustomerId,
                            Notificationmessage =
                                $"The nft(s), has minted and send to the address {mas.Receiveraddress}",
                            State = "new",
                            Color = "success"
                        };
                    }
                ...
                if (state == "success")
                {
                    try
                    {
                        await NftReservationClass.MarkAllNftsAsSold(db, mas.Reservationtoken, newMintAndSendClass.NftProject.Cip68, mas.Receiveraddress);
                    }
                    catch 
                    {
