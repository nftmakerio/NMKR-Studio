    ...
    public static class Command
    {
            ...
            Func<int, bool> handleExitCode = null)
        {
            ...
            try
            {
                ...
                Task.WaitAll(new[] {runProcess, readOutput}, 100000);

                if (process.ExitCode != 0)
                {
                    var readError = process.StandardError.ReadToEndAsync();
                    Task.Delay(5000);
                  //  Console.WriteLine("Wait for errormessage ");
                    Task.WaitAll(new[] {runProcess, readError}, 100000);
